class ModelGurufin {
     ...
     * In Gurufin, this checks for Guard Band activation.
     * Recentering the 'Virtual Peg' (p0 = p) is now conditional based on Config.
     */
    setOraclePrice(p) {

        this.currentOraclePrice = p;

        if (Config.sim.recenterGurufin) {
            this.p0 = p;
        }
    }
     ...
     * @param {number} amountInRaw - Amount of input token
     * @param {boolean} isBuyKRW - True if buying KRW (input USD), False if selling KRW (input KRW)
     * @param {boolean} isArbitrage - True if this is an arbitrage trade (bypasses Safe Mode)
     */
    swap(amountInRaw, isBuyKRW, isArbitrage = false) {
