const DataLoader = {
    ...
    /**
     * Validates the realized volatility of generated GBM data
     * @param {Array} path - Generated price path
     * @param {number} expectedVol - Expected annual volatility
     * @param {number} stepsPerDay - Steps per day
     * @returns {Object} - Validation results with realized volatility
     */
    validateVolatility: function(path, expectedVol, stepsPerDay) {
        if (path.length < 2) return { valid: false, message: "Insufficient data" };

        let logReturns = [];
        for (let i = 1; i < path.length; i++) {
            const logReturn = Math.log(path[i].price / path[i-1].price);
        ...
        const annualizedVol = stdDev * Math.sqrt(365 * stepsPerDay);

        const tolerance = 0.005;
        const isValid = Math.abs(annualizedVol - expectedVol) < tolerance;

        return {
            valid: isValid,
            expectedVol: (expectedVol * 100).toFixed(2) + '%',
            realizedVol: (annualizedVol * 100).toFixed(2) + '%',
            difference: ((annualizedVol - expectedVol) * 100).toFixed(2) + '%',
            message: isValid ?
                `Volatility validation passed (Expected: ${(expectedVol * 100).toFixed(2)}%, Realized: ${(annualizedVol * 100).toFixed(2)}%)` :
                `Volatility validation warning (Expected: ${(expectedVol * 100).toFixed(2)}%, Realized: ${(annualizedVol * 100).toFixed(2)}%)`
        };
    },
